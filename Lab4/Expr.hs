...
-- | Function for checking if an expression is valid, i.e consists of sensible operators or function names.
isValidExpr :: Expr -> Bool
isValidExpr (Bin a "+" b) = isValidExpr a && isValidExpr b
isValidExpr (Bin a "*" b) = isValidExpr a && isValidExpr b
isValidExpr (Function "sin" e) = isValidExpr e
isValidExpr (Function "cos" e) = isValidExpr e
isValidExpr Var = True
isValidExpr (Num _) = True
isValidExpr _ = False -- Anything else is considered invalid

-- | Property for validating that arbitrarily generated expressions are in fact valid ones
prop_validExpr :: Expr -> Bool
prop_validExpr = isValidExpr
...
eval (Bin a "+" b) v      = eval a v + eval b v
eval (Bin a "*" b) v      = eval a v * eval b v
eval (Bin _ op _) _       = error $ "Unsupported binary operator: " ++ op
eval (Function "cos" a) v = cosDouble $ eval a v
eval (Function "sin" a) v = sinDouble $ eval a v
